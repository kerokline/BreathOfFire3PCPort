...
area as OUT/NAME.png a short moment after each. It ends the game when the
recipe says done or FAILED, or after --minutes.
...
SHOT = re.compile(r'input\s+shot (\S+) recipe frame (\d+)')
...
def main():
    ...
    ap.add_argument('--delay', type=float, default=0.4,
                    help='seconds between a shot line and the grab, for the frame to be presented')
        ...
        while status is None and time.time() < deadline:
            ...
            for line in chunk[:cut].decode('utf-8', errors='replace').splitlines():
                if 'FATAL' in line:
                    ...
                    time.sleep(a.delay)
                    print(f'shot {m[1]} at recipe frame {m[2]}')
