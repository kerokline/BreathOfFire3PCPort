...
// deadline no debt to replay. Pause returns false when the clock is not ours
// (BOF3X_ORIGINAL=Game_Clock), when a pause is repaid as usual, or is already
// paused; Resume does nothing unless paused.
...
// (GameClock_Inject) steps 15.6 ms at a time (GetTickCount's granularity), which at a
