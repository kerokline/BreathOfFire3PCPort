...
extern "C" void __cdecl Menu_DrawIcon(int x, int y, int icon, int shade) {
...
// original 0x573F30 (PSX 0x801D90E8): Menu_DrawIcon, with icon 4 drawn as
...
extern "C" unsigned __cdecl Item_Price(unsigned category, unsigned id) {
...
extern "C" void __cdecl Shop_DrawBuyList(unsigned char* window) {
    ...
    if (list[0] != 0) {
        ...
        do {
            ...
            const unsigned percent = w[0xB];   // pushed before Item_Price, for the scale after it
