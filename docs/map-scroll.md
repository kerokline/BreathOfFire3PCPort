...
```
Cond_ByteFF = Field_StatusBits & 1                       // 0x8034E1, bit 0
...
| `0x8034E1` | `Field_StatusBits` | bit 0 goes to `Cond_ByteFF` at every set-up; group D named the byte for its camera-turn bits (hypothesis: nothing else about the byte is read here; 41 functions touch it) |
...
- `Field_StatusBits` `0x8034E1`: what the byte means, and who writes it.
