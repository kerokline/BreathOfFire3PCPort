...
**Forty-one functions of ~2,952 are ours**: `LoadDatFile` `0x454590`, the DAT
...
the regenerated takeover queue gave the first sixteen **logic** functions,
...
attract run has - `AreaMap_ByteAt`, `Sprite_PointInReach` and
`Sprite_RestoreClut`. Reading them fixed the **sprite object arrays - 30 + 4
objects of `0xA4` bytes at `0x7DEE80` / `0x802000`**, which 53 functions
reference. All sixteen pass the oracle, the memory dumps and a re-recorded frame
hash. Three of the forty-one (`Gfx_UploadLzss`, `Gfx_MoveImage`,
...
- 75 functions, 8 global blocks and 52 data items named in
  [`symbols.toml`](../symbols.toml), tiered; 59 functions carry signatures and
  are callable from our code, 41 of them ours (counted 2026-09-20 by
...
1. **Replace every function the attract sequence reaches.** It is the part of
   ...
   with the takeover queue already layered (§9 there). Forty-one are ours, not
