...
| `0x5809C0` | `Save_QuickWrite`, **F12's save** (read whole 2026-09-23): sets the slot `0x9036D4` to **0**, builds the block (`0x5806F0`, the menu's builder), writes `0x12B0` bytes to `BISLPS00.DAT` (format `0x664068` `BISLPS%02X.DAT`) and on success copies the 0x1C-byte summary `0x904680` into slot 0's list entry `0x905BC0`. Sole caller the window procedure's F12 (`0x4FC9A1`), which then shows "Save OK". A save file anywhere, anytime, always slot 0, no confirmation - not a save state. **Tested by the owner 2026-09-23**: pressed anywhere, including in a battle, it makes a normal save that loads in the field where the party stood, out of combat - the block holds the field's area and position, and no battle state at all. So it is a save-anywhere, and in a fight an escape. Mid-event (a cutscene) still untested | evidence |
