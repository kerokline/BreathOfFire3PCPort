...
| `0x573E50` | `Menu_DrawBigIcon` | 0xDE | `0x801D8F9C` call | a 0x28 x 0x30 icon of table `0x66367C` |
...
| `0x5749F0` | `Item_BasePrice` | 0x6E | `0x801DA988` call-anchored | an item's price by category |
...
key items); prices in each item record (`Item_BasePrice`); the experience table
...
- **Reads after calls.** Where the original reads a field after a call, ours
  ...
  (`Shop_DrawBuyList` pushes the price percent `+0xB` before `Item_BasePrice` and
...
`Item_CanUse`, `Menu_ListScroll`; eax for `Item_BasePrice`, `Char_ExpForLevel`,
