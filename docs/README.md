...
| [`sprite-draw-order.md`](sprite-draw-order.md) | The field's sprite structures, found by working the takeover queue: **30 + 4 sprite objects of `0xA4` bytes at `0x7DEE80` / `0x802000`**, the 40-entry draw list and the pass `0x593060` that orders it over 0x37 layers, the 56 x 28 view-cell grid, the draw-item index pool, the animation script format. Nineteen small functions ours, fuzzed against clones and live-checked; the pass itself read, not taken over. In progress. |
| [`psx-library-layer.md`](psx-library-layer.md) | The porting house wrote Sony's libraries rather than rewrite the game's calls into them: libgpu setters and ordering-table links, `getTPage` / `getClut`, a sine, and a GTE whose registers are globals at `0x7DE4xx` with a 20-deep matrix stack. 29 integer functions ours - the hottest of the attract run, `ApplyMatrix` at 2.6 M calls - and the x87 ones deliberately not. Also where `-fno-strict-aliasing` was found necessary. In progress. |
