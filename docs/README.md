...
| [`dialogue-localisation.md`](dialogue-localisation.md) | The plan for stage 2's dialogue box: per-language overlay `DAT`s and the donor disc's glyph atlases upscaled into the port's global 24 px table, all built locally. What is measured - one global font, glyph-code script on the PSX stepper, 256 message slots in JP / EN / ZH alike, PSP discs are PSX EMIs - and what is not, advance width first. Draft. |
