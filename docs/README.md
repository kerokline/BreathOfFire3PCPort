...
| [`sprite-draw-order.md`](sprite-draw-order.md) | The field's sprite structures, found by working the takeover queue: **30 + 4 sprite objects of `0xA4` bytes at `0x7DEE80` / `0x802000`**, the 40-entry draw list and the pass `0x593060` that orders it over 0x37 layers, the 56 x 28 view-cell grid, the draw-item index pool, the animation script format. Sixteen small functions ours, fuzzed against clones and live-checked; the pass itself read, not taken over. In progress. |
