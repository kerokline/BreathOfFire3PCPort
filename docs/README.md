...
| [`sprite-draw-order.md`](sprite-draw-order.md) | The pass `0x593060` that orders field sprites for drawing: a 40-entry list sorted by layer and depth, merged per layer with a second table, 0x37 layers. Its two exchange helpers are ours, fuzzed against clones; the pass itself is read, not taken over. In progress. |
